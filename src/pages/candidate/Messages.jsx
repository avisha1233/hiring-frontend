...
export default function Messages() {
  ...
  useEffect(() => {
    if (!authUser?.id) {
      ...
      setError("No authenticated candidate found.");
      return;
    }
    ...
    async function loadConversations() {
      ...
      try {
        ...
        const candidate = unwrapObject(profile);
        const resolvedCandidateId = Number(
          candidate?.id || candidate?.candidate_id || candidate?.candidateId,
        );
        ...
        const conversationsResponse = await api.get("/conversations", {
          params: { candidate_id: resolvedCandidateId },
        });
        ...
        const summaries = await Promise.all(
          conversationList.map(async (conversation) => {
            const [companyResult, latestMessageResult] =
              await Promise.allSettled([
                api.get(`/companies/${conversation.company_id}`),
                api.get("/messages", {
                  params: {
            ...
            const company =
              companyResult.status === "fulfilled"
                ? unwrapObject(companyResult.value.data)
                : {};
            ...
            return [
              conversation.id,
              {
                ...
                companyAvatar: company?.logo_url || "",
  ...
  const handleSend = async () => {
    ...
    if (!content || !activeConversationId || !candidateId) {
      return;
    }
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-sm text-gray-600">
            Keep conversations with recruiters in one place.
          </p>
        </div>
        <EmptyState
          title="No conversations yet"
          message="When a company starts a chat, it will appear here."
          icon={MessageCircle}
        />
      </div>
    );
        ...
        <aside className="overflow-hidden rounded-2xl border border-orange-100 bg-white shadow-sm">
          <div className="border-b border-orange-100 bg-orange-50 px-4 py-3">
            ...
            <p className="text-xs text-gray-600">
              {candidateId ? `Candidate #${candidateId}` : "Candidate messages"}
            </p>
          </div>
        ...
        <div className="overflow-hidden rounded-2xl border border-orange-100 bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-orange-100 bg-gradient-to-r from-orange-50 to-white px-5 py-4">
            <div className="flex items-center gap-3">
              <Avatar
                name={
                  activeSummary.companyName ||
                  `Company ${activeConversation?.company_id}`
                }
                size="md"
              />
              <div>
