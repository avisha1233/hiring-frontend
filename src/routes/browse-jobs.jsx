...
const CATEGORIES = ["Design", "Development", "Marketing", "Sales", "Product", "Data Science"];
...
function BrowseJobsPage() {
  ...
  useEffect(() => {
    setPage(1);
  }, [search, locationFilter, selectedCategory, selectedTypes.join(",")]);

  const jobsQuery = useQuery({
    queryKey: ["candidate", "jobs", search, locationFilter, selectedCategory, selectedTypes.join(","), page],
    queryFn: () => jobsApi.getJobs({ page, limit: 20, search }),
    retry: false,
    staleTime: 30_000,
  });
  ...
  const jobs = useMemo(() => {
    ...
    return rows.filter((job) => {
      const searchable = [job.title, job.description, job.location, job.category, job.company_name]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();

      const keywordMatches = !keyword || searchable.includes(keyword);
      const categoryMatches = !category || searchable.includes(category);
      const locationMatches = !location || String(job.location || "").toLowerCase().includes(location);
      const typeLabel = job.is_remote ? "Remote" : "Full Time";
      const typeMatches = selectedTypes.length === 0 || selectedTypes.includes(typeLabel);

      return keywordMatches && categoryMatches && locationMatches && typeMatches;
    });
  }, [jobsQuery.data?.data, search, locationFilter, selectedCategory, selectedTypes]);
    ...
    >
      <section className="grid grid-cols-1 gap-4 lg:grid-cols-[280px,1fr]">
        <aside className="rounded-xl border border-(--dash-border) bg-(--dash-surface) p-4 shadow-(--dash-shadow)">
          <h3 className="m-0 text-sm font-semibold text-(--dash-text)">Filters</h3>

          <label className="mt-3 block text-sm text-(--dash-muted)">
            Location
            <input
              value={locationFilter}
              onChange={(event) => setLocationFilter(event.target.value)}
              placeholder="Kathmandu, Remote, etc."
              className="mt-1 h-10 w-full rounded-lg border border-(--dash-border) bg-white px-3 text-sm text-(--dash-text) outline-none"
            />
          </label>

          <h3 className="m-0 mt-5 text-sm font-semibold text-(--dash-text)">Categories</h3>
          <div className="mt-3 space-y-2">
            {CATEGORIES.map((category) => (
              <label key={category} className="flex items-center gap-2 text-sm text-(--dash-muted)">
                <input
                  type="radio"
                  name="category"
          ...
          <h3 className="m-0 mt-5 text-sm font-semibold text-(--dash-text)">Job Type</h3>
          <div className="mt-3 space-y-2">
            {JOB_TYPES.map((type) => {
              const checked = selectedTypes.includes(type);
              return (
                <label key={type} className="flex items-center gap-2 text-sm text-(--dash-muted)">
                  <input
                    type="checkbox"
                    checked={checked}
                    onChange={() => {
        ...
        <div className="rounded-xl border border-(--dash-border) bg-(--dash-surface) p-4 shadow-(--dash-shadow)">
          <div className="mb-4 flex items-center justify-between">
            <h3 className="m-0 text-sm font-semibold text-(--dash-text)">Available Jobs</h3>
          ...
          {jobsQuery.isPending ? (
            ...
            <div className="space-y-3">
              {jobs.map((job) => {
                ...
                return (
                  <article key={job.id} className="rounded-xl border border-(--dash-border) bg-(--dash-bg-elevated) p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="m-0 text-sm font-semibold text-(--dash-text)">{job.title || "Untitled role"}</p>
                        <p className="m-0 mt-1 text-xs text-(--dash-muted)">Company #{job.company_id}</p>
                      </div>
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          setBookmarked((prev) => ({ ...prev, [job.id]: nextValue }));
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-(--dash-muted)">
                      ...
                      <span className="rounded-md border border-(--dash-border) bg-white px-2 py-1">
                        {job.required_experience ? `${job.required_experience}+ yrs` : "0-2 yrs"}
                      </span>
                    </div>
                  </article>
                );
