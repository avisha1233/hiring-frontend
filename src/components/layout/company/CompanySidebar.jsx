...
export default function Sidebar({ badges = {}, userName = "", userInitials = "TC" }) {
  ...
  return (
    <div style={{ width: "210px", minWidth: "210px", background: "#fff", borderRight: "0.5px solid #FFD0B0", display: "flex", flexDirection: "column", height: "100vh" }}>
        ...
        {links.map((item) => {
          ...
          return (
            <Link
              key={item.path}
              to={item.path}
              style={{ display: "flex", alignItems: "center", gap: "9px", padding: "7px 10px", borderRadius: "8px", fontSize: "12px", color: active ? "#C2570A" : "#6b7280", fontWeight: active ? 500 : 400, background: active ? "#FFF5EE" : "transparent", textDecoration: "none" }}
            >
      ...
      <div style={{ padding: "10px", borderTop: "0.5px solid #FFE8D6" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "8px", padding: "7px", borderRadius: "8px", cursor: "pointer" }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: "12px", fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{userName}</div>
